...
const AgentSellMembershipPage = () => {
  ...
  const [success, setSuccess] = useState<{ plan: string; endDate: string; discount: number } | null>(null)
  ...
  return (
    <>
      <PageTitle title="Sell Membership" subTitle="Subscribe a customer to a membership plan" />
        ...
        <Col xs={12} md={8}>
              ...
              <Col key={plan.key} xs={12} sm={4}>
                ...
                >
                  <CardBody className="text-center">
                    ...
                    <div className="fs-4 fw-bold text-primary mb-2">LKR {plan.price}</div>
          ...
          {selectedPlan && (
            <Card className="mb-3 bg-light border-0">
              <CardBody>
                <p className="mb-1"><strong>Plan:</strong> {selectedPlan.label}</p>
                <p className="mb-1"><strong>Price:</strong> LKR {selectedPlan.price} (deducted from customer wallet)</p>
                <p className="mb-0"><strong>Discount:</strong> {selectedPlan.discount}% off all orders for {selectedPlan.duration}</p>
              </CardBody>
            </Card>
          )}
          ...
          {success && (
            <div className="mt-4 p-3 bg-success bg-opacity-10 rounded border border-success">
              <p className="mb-1 fw-semibold text-success">✓ Membership activated</p>
              <p className="mb-1 text-muted">Plan: {success.plan}</p>
              <p className="mb-1 text-muted">Expires: {success.endDate}</p>
              <p className="mb-0 fw-bold">{success.discount}% discount applied on all future orders</p>
            </div>
          )}
        </Col>
      </Row>
