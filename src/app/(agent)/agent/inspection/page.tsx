...
import { alertsApi, machinesApi } from '@/lib/api'
...
const severityBg = (s?: string) =>
  ({ critical: 'danger', high: 'warning', medium: 'info', low: 'secondary' })[s ?? ''] ?? 'secondary'

// ── Inspection Modal ──────────────────────────────────────────────────────────
const InspectionModal = ({
  show, onHide, machine, token, onDone,
}: {
  show: boolean; onHide: () => void; machine: Machine | null; token: string; onDone: () => void
}) => {
  ...
  useEffect(() => {
    if (show) { setChecks({}); setIssues(''); setSeverity('medium') }
  }, [show])

  const toggle = (id: string) => setChecks((p) => ({ ...p, [id]: !p[id] }))
  const allPassed = CHECKLIST.every((c) => checks[c.id])
  const failedItems = CHECKLIST.filter((c) => !checks[c.id]).map((c) => c.label)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const hasIssues = failedItems.length > 0 || issues.trim()
      if (hasIssues) {
        const failedStr = failedItems.length > 0 ? `Failed checks: ${failedItems.join(', ')}. ` : ''
        const noteStr = issues.trim() ? `Notes: ${issues.trim()}` : ''
        await fetch(
          `${API_BASE}/alerts`,
          {
            method: 'POST',
        ...
        toast.warning(`Inspection filed — ${failedItems.length} issue(s) reported`)
      } else {
        ...
        <Modal.Body>
          {machine?.sensor && (
            <div className="alert alert-light py-2 mb-3 fs-12">
              {machine.sensor.temp != null && <span className="me-3"><i className="ri-temp-hot-line me-1" />{machine.sensor.temp}°C</span>}
              {machine.sensor.water && <span className="me-3 text-capitalize"><i className="ri-drop-line me-1" />Water: {machine.sensor.water}</span>}
            </div>
          )}
          ...
          <Form.Group className="mt-3 mb-2">
            <Form.Label className="fw-semibold fs-13">Issues / Notes</Form.Label>
            <Form.Control
              as="textarea"
              rows={3}
          ...
          {(failedItems.length > 0 || issues.trim()) && (
            <Form.Group className="mb-2">
              <Form.Label className="fw-semibold fs-13">Alert Severity</Form.Label>
          ...
          {allPassed && !issues.trim() && (
            <div className="alert alert-success py-2 fs-13 mt-2">
              <i className="ri-checkbox-circle-line me-1" />All checks passed — no alert will be created.
            </div>
          )}
        </Modal.Body>
        <Modal.Footer>
          ...
          <Button
            ...
            size="sm"
            disabled={saving}>
            {saving ? <Spinner size="sm" className="me-1" /> : <i className="ri-send-plane-line me-1" />}
            Submit Inspection
          </Button>
        </Modal.Footer>
      </Form>
      ...
      <Row className="mb-3 align-items-center">
        <Col>
          <div className="d-flex gap-2">
            <Badge bg="secondary" className="fs-13 fw-normal px-3 py-2">Machines: {machines.length}</Badge>
            {pendingAlerts.length > 0 && (
              <Badge bg="warning" text="dark" className="fs-13 fw-normal px-3 py-2">
      ...
      <Card className="mb-3">
        ...
        <CardBody className="p-0">
          {loading ? (
            ...
            <div className="text-center text-muted py-4">No machines assigned</div>
          ) : (
                ...
                <tbody>
                  {machines.map((m) => (
                    <tr key={m._id}>
                      ...
                      <td>
                        {m.sensor?.water ? (
                          <Badge bg={m.sensor.water === 'full' ? 'success' : m.sensor.water === 'low' ? 'warning' : 'danger'} className="text-capitalize">
                            {m.sensor.water}
                          </Badge>
                      ...
                      <td>
                        {m.sensor?.temp != null ? (
                          <span className={m.sensor.temp > 70 ? 'text-danger fw-bold' : 'text-muted'}>
                            {m.sensor.temp}°C
                          </span>
                        ) : <span className="text-muted">—</span>}
                      </td>
                      <td className="text-center">
                        <Button
                          ...
                          <i className="ri-clipboard-check-line me-1" />Inspect
                        </Button>
                      </td>
                    </tr>
                  ))}
                ...
                <tbody>
                  {alerts.slice(0, 20).map((a) => (
                    <tr key={a._id}>
                      ...
                      <td style={{ maxWidth: 280, whiteSpace: 'normal' }}>{a.message ?? '—'}</td>
                      <td><Badge bg={severityBg(a.severity)} className="text-capitalize">{a.severity ?? '—'}</Badge></td>
