...
const EmailLoginForm = () => {
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    if (res?.ok) {
      push('/dashboard')
      showNotification({ message: 'Logged in successfully', variant: 'success' })
    } else {
      showNotification({ message: res?.error ?? 'Invalid credentials', variant: 'danger' })
    }
  })
    ...
    <form onSubmit={onSubmit} className="text-start mb-3">
      <div className="mb-3">
        <TextFormInput control={control} name="email" placeholder="Enter your email"
          className="bg-light bg-opacity-50 border-light py-2" label="Email" />
      </div>
      <div className="mb-3">
        <TextFormInput control={control} name="password" placeholder="Enter your password"
          className="bg-light bg-opacity-50 border-light py-2" label="Password" />
      </div>
...
const PhoneOtpForm = () => {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStep('otp')
      showNotification({ message: 'OTP sent to your phone', variant: 'success' })
    } catch (err: any) {
      ...
      {step === 'phone' ? (
        ...
        <form onSubmit={handleVerifyOtp}>
          <div className="mb-3 text-muted fs-13">
            ...
            <button type="button" className="btn btn-link btn-sm p-0 fs-13" onClick={() => { setStep('phone'); setOtp('') }}>
              Change
            </button>
          </div>
          <Form.Group className="mb-3">
            ...
            <Form.Control
              ...
              required
            />
            <Form.Text className="text-muted">OTP expires in 5 minutes</Form.Text>
          </Form.Group>
          {error && <div className="alert alert-danger py-2 fs-13">{error}</div>}
          <Button type="submit" disabled={loading} className="w-100 fw-semibold">
            {loading ? <Spinner size="sm" className="me-1" /> : null} Verify & Login
          </Button>
        </form>
      )}
    </div>
      ...
      <Row className="g-0 justify-content-center w-100 m-xxl-5 px-xxl-4 m-3">
        <Col xl={3} lg={4} md={6}>
          <Card className="overflow-hidden text-center rounded-4 p-xxl-4 p-3 mb-0">
            ...
            <p className="text-muted mb-4">Sign in to manage vending operations.</p>

            <Tabs defaultActiveKey="email" className="mb-3 nav-pills-custom" fill>
              <Tab eventKey="email" title={<><i className="ri-mail-line me-1" />Email</>}>
                <EmailLoginForm />
              </Tab>
              <Tab eventKey="phone" title={<><i className="ri-phone-line me-1" />Phone OTP</>}>
                <PhoneOtpForm />
              </Tab>
            </Tabs>
