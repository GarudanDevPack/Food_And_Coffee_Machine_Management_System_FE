...
const CoffeeStockModal = ({
  ...
  useEffect(() => {
    if (!token) return;
    itemsApi
      .list(token, { itemType: "coffee" })
      .then((d) => setCoffeeItems(d as Item[]))
      .catch((err: any) =>
        toast.error(err?.message ?? "Failed to load coffee items"),
      );
  }, [token]);
  ...
  useEffect(() => {
    if (editingInv) {
      setForm({
        ...
        qty:
          editingInv.gramsPerCup && editingInv.currentStock
            ? String(
                Math.round(editingInv.currentStock / editingInv.gramsPerCup),
              )
            : "",
        cupsize: "120",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!packet || !qty) {
      toast.warning("Packet weight and cup count are required");
      return;
    }
    const ratio = packet / qty;
    if (ratio < 3 || ratio > 29) {
      toast.warning(
        `Grams per cup must be 3–29g. Current: ${ratio.toFixed(2)}g`,
      );
      return;
    }
    setSaving(true);
  ...
  return (
    <Modal show={show} onHide={onHide} centered>
      <Modal.Header closeButton>
        <Modal.Title>
          {isEdit ? "Update Coffee Stock" : "Add Item to Inventory"}
        </Modal.Title>
      </Modal.Header>
        ...
        <Modal.Body>
          ...
          <Row className="mb-3">
            <Col>
              <Form.Group>
                <Form.Label>
                  Packet Weight (g) <span className="text-danger">*</span>
                </Form.Label>
                <Form.Control
                  type="number"
                  min={1}
                  value={form.packetofstock}
                  onChange={set("packetofstock")}
                  placeholder="e.g. 250"
                  onFocus={() =>
                    toast.info("Enter the total packet weight in grams", {
                      autoClose: 3000,
                    })
                  }
                  required
            ...
            <Col>
              <Form.Group>
                <Form.Label>
                  Cup Count <span className="text-danger">*</span>
                </Form.Label>
                <Form.Control
                  type="number"
                  min={1}
                  ...
                  placeholder="e.g. 14"
                  onFocus={() =>
                    toast.info("Number of cups this packet makes", {
                      autoClose: 3000,
                    })
                  }
                  required
            ...
            <Col>
              <Form.Group>
                ...
                <Form.Control
                  ...
                  className="bg-light"
                  onFocus={() =>
                    toast.info("Grams used per cup (auto-calculated)", {
                      autoClose: 3000,
                    })
                  }
                />
...
const LoadBatchModal = ({ show, onHide, machineId, token, onSaved }: any) => {
  ...
  useEffect(() => {
    if (!token) return;
    itemsApi
      .list(token, { itemType: "food" })
      .then((d) => setFoodItems(d as Item[]))
      .catch((err: any) =>
        toast.error(err?.message ?? "Failed to load food items"),
      );
  }, [token]);
  ...
  const handleItemChange = (e: any) => {
    ...
    const item = foodItems.find(
      (i) => String(i._id ?? i.id) === id || i.name === selectedName,
    );
    setForm((p) => ({
      ...p,
      itemId: id,
  ...
  return (
    <Modal show={show} onHide={onHide} centered>
      <Modal.Header closeButton>
        <Modal.Title>Load Food Batch</Modal.Title>
      </Modal.Header>
      <Form onSubmit={handleSubmit}>
        <Modal.Body>
          <Form.Group className="mb-3">
...
export default function StockPage() {
  ...
  const selectedMachine = machines.find(
    (m) => m.machineId === selectedMachineId,
  );

  const fetchBatches = () => {
    if (!selectedMachine || selectedMachine.machineType !== "food") {
      setBatches([]);
      return;
    }
    setBatchLoading(true);
    machinesApi
      .getBatches(token, selectedMachine.machineId)
  ...
  const batchStatusBg = (s?: string) =>
    s === "active" ? "success" : s === "expired" ? "danger" : "secondary";
    ...
    <>
        ...
        <Card>
          <div className="d-flex card-header align-items-center">
            <h5 className="mb-0 me-auto">
              Coffee Inventory — {selectedMachine.name}
            </h5>
            <Button
              variant="primary"
              size="sm"
                  ...
                  {!selectedMachine.inventory?.length ? (
                    <tr>
                      <td colSpan={6} className="text-center text-muted py-4">
                        No inventory — click "Add Item" to load stock
                      </td>
                    </tr>
                  ) : (
                    selectedMachine.inventory.map((inv) => (
                      <tr key={inv.itemId}>
                        <td className="fw-semibold">
                          {inv.itemName ?? (
                            <code className="fs-12">{inv.itemId}</code>
                          )}
                        </td>
                        <td className="text-center">
                          <Badge bg={stockBadge(inv)}>
                            {inv.currentStock}g
                          </Badge>
                        </td>
      ...
      {selectedMachine && selectedMachine.machineType === "food" && (
        <Card>
          <div className="d-flex card-header align-items-center">
            <h5 className="mb-0 me-auto">
              Food Batches — {selectedMachine.name}
            </h5>
            <Button
              variant="primary"
              size="sm"
              onClick={() => setShowLoadBatch(true)}
            >
                  ...
                  <tbody>
                    {batches.length === 0 ? (
                      ...
                      batches.map((b) => (
                        <tr key={b.batchId}>
                          <td>
                            <code className="fs-12">{b.batchId}</code>
                          </td>
