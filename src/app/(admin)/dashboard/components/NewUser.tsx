...
// Backend: { _id: itemId, itemName, count, revenue }
...
const normaliseTopItems = (raw: RawTopItem[]): TopItem[] =>
  raw.map((r) => ({
    itemId: r._id,
    name: r.itemName,
    totalOrders: r.count,
...
const NewUser = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([items, orders]) => {
        setTopItems(normaliseTopItems(items as RawTopItem[]));
                    ...
                    {topItems.map((item, idx) => (
                      <tr key={item.itemId}>
                        ...
                        <td className="text-end text-success fw-semibold">
                          LKR {(item.totalRevenue ?? 0).toLocaleString()}
                        </td>
                      </tr>
                    ))}
                  </tbody>
