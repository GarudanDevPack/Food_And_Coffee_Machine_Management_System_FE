    ...
    <>
      ...
      <Row className="mb-3">
        {[
          ...
          { label: 'Expired', value: memberships.filter((m) => m.status === 'expired').length, bg: 'danger' },
              ...
              <Nav.Item key={s}>
                <Nav.Link active={tab === s} onClick={() => setTab(s)} style={{ cursor: 'pointer', textTransform: 'capitalize' }}>
                  ...
                  {s === 'active' && activeCount > 0 && (
                    <Badge bg="success" className="ms-1">{activeCount}</Badge>
                  )}
                </Nav.Link>
              </Nav.Item>
            ))}
          </Nav>
          <Button variant="light" size="sm" onClick={fetchMemberships} disabled={loading}>
          ...
          {loading ? (
            ...
            <div className="table-responsive">
              <Table className="table-custom table-centered table-sm table-nowrap table-hover mb-0">
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr><td colSpan={8} className="text-center text-muted py-4">No memberships found</td></tr>
                  ) : filtered.map((m, i) => (
                    <tr key={m._id ?? m.id ?? i}>
                      <td className="fw-semibold">{getUserLabel(m.userId)}</td>
                      <td>
                      ...
                      <td className="text-center">
                        <Badge bg="warning" text="dark">{m.discount ?? 0}%</Badge>
                      </td>
                      <td className="text-muted fs-12">
                        {m.startDate ? new Date(m.startDate).toLocaleDateString('en-GB') : '—'}
                      </td>
                      <td className={`fs-12 ${isExpired(m.endDate) ? 'text-danger fw-semibold' : 'text-muted'}`}>
                        {m.endDate ? new Date(m.endDate).toLocaleDateString('en-GB') : '—'}
                        {isExpired(m.endDate) && <i className="ri-alarm-warning-line ms-1" />}
                      </td>
                      <td className="text-end fw-semibold">LKR {(m.pricePaid ?? 0).toLocaleString()}</td>
                      <td>
                        <Badge bg={statusBg(m.status)} className="text-capitalize">{m.status ?? '—'}</Badge>
                      </td>
                      <td className="text-center">
                        {m.status === 'active' ? (
                          <Button variant="soft-danger" size="sm" onClick={() => handleCancel(m)} title="Cancel Membership">
