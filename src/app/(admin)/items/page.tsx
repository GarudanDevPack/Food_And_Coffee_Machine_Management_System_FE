...
const ItemModal = ({ show, onHide, item, token, onSaved }: any) => {
  ...
  const [cupSizes, setCupSizes] = useState<{ size: string; price: string }[]>([
    { size: "small", price: "" },
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.itemType === "food") {
        ...
        payload.cupSizes = cupSizes
          .filter((c) => c.size && c.price)
          .map((c) => ({ size: c.size, price: Number(c.price) }));
      }
        ...
        <Modal.Body>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Control
              ...
              placeholder="e.g. hot, cold, snack"
            />
          </Form.Group>
          <Form.Group className="mb-3">
                ...
                <Row key={idx} className="mb-2 g-2 align-items-center">
                  ...
                  <Col>
                    <Form.Control
                      ...
                      onChange={(e) =>
                        setCupSizes((p) =>
                          p.map((c, i) =>
                            i === idx ? { ...c, price: e.target.value } : c,
                          ),
                        )
                      }
                    />
                  ...
                  <Col xs="auto">
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        setCupSizes((p) => p.filter((_, i) => i !== idx))
                      }
                      disabled={cupSizes.length === 1}
                    >
                      ✕
                    </Button>
                  </Col>
          ...
          <Form.Check
            ...
            onChange={(e) =>
              setForm((p: any) => ({ ...p, isAvailable: e.target.checked }))
            }
          />
        </Modal.Body>
        <Modal.Footer>
...
export default function ItemsPage() {
  ...
  const handleDelete = async (item: Item) => {
    const result = await Swal.fire({
      title: "Delete Item?",
      text: `"${item.name}" will be marked unavailable.`,
      icon: "warning",
                  ...
                  {filtered.length === 0 ? (
                    ...
                    filtered.map((item) => (
                      <tr key={item._id ?? item.id}>
                        ...
                        <td>
                          <Badge
                            bg={item.itemType === "food" ? "warning" : "info"}
                            className="text-capitalize"
                          >
                        ...
                        <td>
                          {item.bayesianRating ? (
                            <span className="text-warning fw-semibold">
                              ★ {(item.bayesianRating as number).toFixed(1)}
                            </span>
                          ) : (
