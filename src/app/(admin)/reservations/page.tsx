...
export default function AdminReservationsPage() {
  ...
  const handleCancel = async (r: Reservation) => {
    ...
    setProcessingId(r._id ?? r.id)
    try {
      await reservationsApi.cancel(token, r._id ?? r.id, result.value || undefined)
  ...
  const displayed = tab === 'all' ? reservations : reservations.filter((r) => r.status === tab)
      ...
      <Row className="mb-3 align-items-center">
        <Col>
          <div className="d-flex gap-2 flex-wrap">
            <Badge bg="secondary" className="fs-13 fw-normal px-3 py-2">Total: {reservations.length}</Badge>
            {pendingCount > 0 && (
              <Badge bg="warning" text="dark" className="fs-13 fw-normal px-3 py-2">Pending: {pendingCount}</Badge>
            )}
          </div>
        </Col>
        <Col xs="auto">
        ...
        <div className="card-header border-bottom">
          <Nav variant="tabs" className="card-header-tabs">
            {([
              ...
              { key: 'confirmed', label: `Confirmed (${countByStatus('confirmed')})` },
              ...
              <Nav.Item key={t.key}>
                <Nav.Link active={tab === t.key} onClick={() => setTab(t.key)} style={{ cursor: 'pointer' }}>
                  {t.label}
                  {t.badge != null && t.badge > 0 && <Badge bg="warning" text="dark" className="ms-1">{t.badge}</Badge>}
                </Nav.Link>
              </Nav.Item>
            ))}
          </Nav>
                ...
                <tbody>
                  {displayed.length === 0 ? (
                    ...
                    <tr key={r._id ?? r.id}>
                      ...
                      <td><code className="text-muted fs-12">{r.machineId ?? '—'}</code></td>
                      ...
                      <td className="text-muted fs-12" style={{ maxWidth: 150, whiteSpace: 'normal' }}>
                        {r.note ?? '—'}
                      </td>
                        ...
                        <div className="d-flex gap-1 justify-content-center">
                          {r.status === 'pending' && (
                            <Button
                              ...
                              title="Confirm">
                              {processingId === (r._id ?? r.id) ? <Spinner size="sm" /> : <i className="ri-check-double-line" />}
                            </Button>
                          )}
                          {r.status === 'confirmed' && (
                            <Button
                              ...
                              {processingId === (r._id ?? r.id) ? <Spinner size="sm" /> : <i className="ri-checkbox-circle-line" />}
                            </Button>
                          )}
