...
const AdminTopupModal = ({ show, onHide, wallet, token, onSaved }: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await walletApi.adminTopup(token, userId, {
      ...
      <Form onSubmit={handleSubmit}>
        <Modal.Body>
          <Form.Group className="mb-3">
            <Form.Label>
              Amount (LKR) <span className="text-danger">*</span>
            </Form.Label>
            <Form.Control
              type="number"
              min={1}
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" type="submit" disabled={saving}>
            {saving ? <Spinner size="sm" className="me-1" /> : null} Apply
            Top-up
          </Button>
        </Modal.Footer>
      </Form>
...
export default function WalletPage() {
  ...
  const handleApprove = async (t: Topup) => {
    const r = await Swal.fire({
      ...
      text: `Credit LKR ${t.amount?.toLocaleString()} to ${getUserName(t.userId)}?`,
      icon: "question",
  ...
  const handleReject = async (t: Topup) => {
    const r = await Swal.fire({
      title: "Reject Top-up?",
      text: `Reject LKR ${t.amount?.toLocaleString()} from ${getUserName(t.userId)}?`,
      icon: "warning",
      input: "text",
        ...
        <Col>
          <div className="d-flex gap-2">
            ...
            <Badge bg="success" className="fs-13 fw-normal px-3 py-2">
              Total Balance: LKR {totalBalance.toLocaleString()}
            </Badge>
                    ...
                    wallets.map((w, i) => (
                      <tr key={w._id ?? w.id ?? i}>
                        ...
                        <td className="text-muted fs-12">
                          {w.updatedAt ? fmtDateTime(w.updatedAt) : "—"}
                        </td>
                        ...
                        <td className="text-center">
                          <div className="d-flex gap-1 justify-content-center">
                            <Button
                              ...
                              disabled={processingId === (t._id ?? t.id)}
                            >
                              {processingId === (t._id ?? t.id) ? (
                                <Spinner size="sm" />
                              ) : (
                                <>
