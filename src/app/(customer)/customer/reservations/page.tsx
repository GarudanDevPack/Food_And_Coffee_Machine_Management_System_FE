...
interface Machine { _id: string; machineId: string; name: string; location?: string }
interface FoodItem { _id: string; name: string; unitPrice?: number }
interface NearExpiryItem { itemId: string; itemName: string; batchId: string; expiryDate: string; hoursRemaining: number; discountPct: number; quantity: number }

const statusBg = (s?: string) =>
  ({ pending: 'warning', confirmed: 'info', completed: 'success', cancelled: 'secondary' })[s ?? ''] ?? 'secondary'
...
// ── Book Modal ────────────────────────────────────────────────────────────────
const BookModal = ({
  show, onHide, token, onBooked,
}: {
  show: boolean; onHide: () => void; token: string; onBooked: () => void
}) => {
  const [machines, setMachines] = useState<Machine[]>([])
  const [items, setItems] = useState<FoodItem[]>([])
  const [nearExpiry, setNearExpiry] = useState<NearExpiryItem[]>([])
  const [form, setForm] = useState({
    machineId: '', itemId: '', itemName: '', date: '', timeSlot: 'lunch', customTime: '', quantity: '1', note: '',
  })
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setMachines(m.filter((x: any) => x.machineType === 'food' || !x.machineType))
      setItems(i)
    }).catch(() => {})
  ...
  useEffect(() => {
    if (!form.machineId || !token) { setNearExpiry([]); return }
    machinesApi.nearExpiryItems(token, form.machineId)
      .then(setNearExpiry)
      .catch(() => setNearExpiry([]))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.machineId || !form.itemId || !form.date) { toast.error('Fill all required fields'); return }
    setSaving(true)
    try {
      ...
      <Form onSubmit={handleSubmit}>
        <Modal.Body>
          <Form.Group className="mb-3">
            ...
            <Form.Select value={form.machineId} onChange={set('machineId')} required>
              ...
              {items.map((i) => {
                ...
                return (
                  <option key={i._id} value={i._id}>
                    {ne ? `🔥 ${i.name} — ${ne.discountPct}% OFF (expires in ${ne.hoursRemaining}h)` : `${i.name}${i.unitPrice ? ` — LKR ${i.unitPrice}` : ''}`}
                  </option>
                )
              })}
            </Form.Select>
            {nearExpiry.some((n) => n.itemId === form.itemId) && (() => {
              ...
              return (
                <div className="mt-2 p-2 rounded bg-warning bg-opacity-10 border border-warning d-flex align-items-center gap-2 fs-13">
                  ...
                  <div>
                    <span className="fw-semibold text-warning">{ne.discountPct}% discount applied automatically</span>
                    <span className="text-muted"> — batch expires in {ne.hoursRemaining}h</span>
                  </div>
                </div>
              )
            })()}
          ...
          <Row>
            ...
            <Col>
              <Form.Group className="mb-3">
                ...
                <Form.Control type="number" min={1} max={10} value={form.quantity} onChange={set('quantity')} />
              </Form.Group>
            </Col>
          </Row>
          ...
          {form.timeSlot === 'custom' && (
            <Form.Group className="mb-3">
              ...
              <Form.Control type="time" value={form.customTime} onChange={set('customTime')} />
            </Form.Group>
          )}
        ...
        <Modal.Footer>
          ...
          <Button type="submit" variant="primary" size="sm" disabled={saving}>
            {saving ? <Spinner size="sm" className="me-1" /> : <i className="ri-calendar-check-line me-1" />}
            Book Reservation
          </Button>
        </Modal.Footer>
      </Form>
...
export default function CustomerReservationsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setReservations((data as Reservation[]).sort((a, b) =>
      ...
      <Row className="mb-3 align-items-center">
        ...
        <Col xs="auto">
          <Button variant="primary" size="sm" onClick={() => setShowBook(true)}>
            <i className="ri-calendar-check-line me-1" />New Reservation
          </Button>
          <Button variant="light" size="sm" className="ms-2" onClick={load} disabled={loading}>
      ...
      {loading ? (
        ...
        <Card>
          <CardBody className="text-center py-5">
            ...
            <Button variant="primary" size="sm" onClick={() => setShowBook(true)}>
              <i className="ri-calendar-check-line me-1" />Book Your First Reservation
            </Button>
          </CardBody>
        </Card>
      ) : (
        <>
          {upcoming.length > 0 && (
            <>
              <h6 className="fw-semibold mb-2 text-muted text-uppercase fs-12">Upcoming</h6>
                    ...
                    <Card className="h-100 border-start border-3 border-primary">
                      <CardBody>
                        <div className="d-flex justify-content-between align-items-start mb-2">
                          ...
                          <Badge bg={statusBg(r.status)} className="text-capitalize">{r.status}</Badge>
                        </div>
                        <div className="text-muted fs-13 mb-1">
                          <i className="ri-map-pin-line me-1" />{r.machineId}
                        </div>
                        <div className="text-muted fs-13 mb-1">
                          <i className="ri-calendar-line me-1" />{r.date}
                        </div>
                        <div className="text-muted fs-13 mb-1">
                          <i className="ri-time-line me-1" />{slotLabel(r.timeSlot, r.customTime)}
                        </div>
                        <div className="text-muted fs-13 mb-2">
                          <i className="ri-stack-line me-1" />Qty: {r.quantity ?? 1}
                        </div>
                        {r.note && <div className="text-muted fs-12 fst-italic mb-2">"{r.note}"</div>}
          ...
          {past.length > 0 && (
            <>
              <h6 className="fw-semibold mb-2 text-muted text-uppercase fs-12">Past</h6>
              <Row className="g-3">
                {past.map((r) => (
                  <Col key={r._id} xs={12} md={6} lg={4}>
                    <Card className="h-100 opacity-75">
                      <CardBody>
                        <div className="d-flex justify-content-between align-items-start mb-2">
                          <div className="fw-semibold text-muted">{r.itemName ?? '—'}</div>
                          <Badge bg={statusBg(r.status)} className="text-capitalize">{r.status}</Badge>
                        </div>
                        <div className="text-muted fs-13">
                          <i className="ri-calendar-line me-1" />{r.date} — {slotLabel(r.timeSlot, r.customTime)}
                        </div>
                      </CardBody>
                    </Card>
                  </Col>
