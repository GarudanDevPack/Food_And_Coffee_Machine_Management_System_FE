...
const CustomerMembershipPage = () => {
  ...
  const handleSubscribe = async () => {
    ...
    if (
      !confirm(
        `Subscribe to ${selectedPlan.label} for LKR ${selectedPlan.price}? This will be deducted from your wallet.`,
      )
    )
      return;
    ...
    try {
      ...
      toast.success(
        `Subscribed to ${selectedPlan.label} plan — ${selectedPlan.discount}% discount active!`,
      );
  ...
  return (
    <>
      <PageTitle
        title="Membership"
        subTitle="Get discounts on all your orders"
      />
                  ...
                  <div>
                    ...
                    <p className="mb-1 text-muted">
                      ...
                      <strong className="text-capitalize">
                        {membership.plan.replace("month", " Month")}
                      </strong>
                    </p>
                    <p className="mb-1 text-muted">
                      Expires:{" "}
                      {new Date(membership.endDate).toLocaleDateString()} (
                      {daysRemaining(membership.endDate)} days left)
                    </p>
                    <p className="mb-0 text-muted">
                      Paid: LKR {membership.pricePaid?.toFixed(2)}
                    </p>
                  </div>
        ...
        <Card className="mb-4 border-warning bg-warning bg-opacity-10">
          <CardBody className="text-center">
            <p className="mb-0 text-warning fw-semibold">
              You don't have an active membership. Subscribe below to get
              discounts!
            </p>
          </CardBody>
        </Card>
      ...
      <h5 className="mb-3">
        {membership ? "Upgrade / Renew Plan" : "Choose a Plan"}
      </h5>
      <Row className="g-3 mb-4">
        {PLANS.map((plan) => (
      ...
      {selectedPlan && (
        <Card className="mb-4 bg-light border-0">
          <CardBody>
            <p className="mb-1">
              <strong>Selected:</strong> {selectedPlan.label}
            </p>
            <p className="mb-0 text-muted">
              LKR {selectedPlan.price} will be deducted from your wallet. You'll
              get {selectedPlan.discount}% off all orders for{" "}
      ...
      >
        ...
        {subscribing
          ? "Subscribing…"
          : `Subscribe — LKR ${selectedPlan?.price ?? "—"}`}
      </Button>
    </>
  );
};
