...
const CustomerWalletPage = () => {
  ...
  const txBadge = (type: string) => {
    const map: Record<string, string> = { credit: 'success', debit: 'danger', refund: 'info' }
    return <Badge bg={map[type] || 'secondary'} className="text-capitalize">{type}</Badge>
  }

  return (
    <>
      <PageTitle title="My Wallet" subTitle="Manage your balance and transactions" />
          ...
          <Card className="text-center h-100">
            <CardBody className="d-flex flex-column align-items-center justify-content-center py-4">
              ...
              {loading ? (
                ...
                <div className="display-5 fw-bold text-primary">LKR {balance?.toFixed(2) ?? '0.00'}</div>
              )}
              <Button variant="outline-secondary" size="sm" className="mt-3" onClick={loadWallet}>
            ...
            <CardBody>
              ...
              <Form onSubmit={handleTopup}>
                <Row className="g-2 align-items-end">
                  ...
                  <Col xs={12} sm={2}>
                    <Button type="submit" variant="primary" className="w-100" disabled={topupLoading}>
                      {topupLoading ? <Spinner size="sm" animation="border" /> : 'Submit'}
                    </Button>
                  </Col>
                </Row>
                ...
                <tbody>
                  {transactions.map((tx) => (
                    <tr key={tx._id}>
                      ...
                      <td className={`text-end fw-semibold ${tx.type === 'debit' ? 'text-danger' : 'text-success'}`}>
