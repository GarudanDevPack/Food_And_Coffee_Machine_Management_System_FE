...
const LayoutProvider = ({ children }: ChildrenType) => {
  ...
  const INIT_STATE: LayoutState = {
    theme: queryParams["layout_theme"]
      ...
      : "light",
    layoutOrientation: queryParams["layout_type"]
      ? (queryParams["layout_type"] as LayoutOrientationType)
      : "vertical",
    topbarTheme: queryParams["topbar_theme"]
      ? (queryParams["topbar_theme"] as TopBarThemeType)
      : "brand",
    menu: {
    ...
    layoutMode: queryParams["layout_mode"]
      ? (queryParams["layout_mode"] as LayoutModeType)
      : "fluid",
  };

  const [settings, setSettings] = useLocalStorage<LayoutState>(
    "__ABSTACK_NEXT_CONFIG__",
    INIT_STATE,
    override,
  );
